...
const createPaymentIntent = async (
  ...
  if (!raffle) {
    throw new AppError(StatusCodes.NOT_FOUND, 'Raffle not found!');
  }
...
const createPaymentIntentCarity = async (
  ...
  if (!charity) {
    throw new AppError(StatusCodes.NOT_FOUND, 'Charity not found!');
  }

  const DonateAmount = Number(amount);
  if (isNaN(DonateAmount)) {
    throw new AppError(StatusCodes.BAD_REQUEST, 'Invalid donation amount');
  }
